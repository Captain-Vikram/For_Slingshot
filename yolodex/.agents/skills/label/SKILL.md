...
| **`gemini`**  | Gemini native bounding box detection (0-1000 scale)        | Fast, good native bbox support (**default**) |
...
1. Read config.json for `label_mode`, `classes`, `model`, `num_agents`
   If mode is missing, default to `gemini`.
...
5. **Parallel dispatch** (GPT or Codex mode):
   ...
   - `label_mode=gemini` with `GEMINI_API_KEY` or `GOOGLE_API_KEY` (runs `run_batch.py`)
...
| `run.py`                 | gemini/gpt       | Provider-aware labeler for single-agent mode                |
| `run_batch.py`           | gemini/gpt       | Provider-aware labeler for subagent batch mode              |
| `dispatch.sh`            | gemini/gpt/codex | Parallel subagent orchestrator                              |
