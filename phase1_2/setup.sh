#!/usr/bin/env bash
# Phase1&2 setup script — installs all dependencies and verifies the environment
set -euo pipefail
...
if [ -n "${GEMINI_API_KEY:-}" ] || [ -n "${GOOGLE_API_KEY:-}" ]; then
  ...
  warn "Gemini API key is not set"
  echo "       Default labeling uses Gemini. Set one of:"
  echo "       export GEMINI_API_KEY=\"...\""
  echo "       export GOOGLE_API_KEY=\"...\""
fi

if [ -n "${OPENAI_API_KEY:-}" ]; then
  ok "OPENAI_API_KEY is set (fallback modes available)"
else
  warn "OPENAI_API_KEY is not set (only needed for gpt/cua+sam modes)"
fi
...
if check_cmd codex; then
  ...
  warn "codex CLI not found (optional — needed for parallel labeling and phase1_2.sh)"
...
chmod +x "$SCRIPT_DIR/phase1_2.sh"
...
echo "Next steps:"
echo ""
echo "  1. Set your Gemini key (default label mode):"
echo "     export GEMINI_API_KEY=\"...\""
echo "     # or: export GOOGLE_API_KEY=\"...\""
echo "     # optional fallback: export OPENAI_API_KEY=\"sk-...\""
echo ""
...
echo "     uv run .agents/skills/label/scripts/run.py    # gemini or gpt based on label_mode"
