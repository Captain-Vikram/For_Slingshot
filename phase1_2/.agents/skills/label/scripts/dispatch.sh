...
WORKTREE_BASE="/tmp/phase1_2-workers"
NUM_AGENTS="${1:-4}"
...
repo_root = Path(os.environ["REPO_ROOT"])
config = json.loads((repo_root / "config.json").read_text(encoding="utf-8"))
print(str(config.get("label_mode", "gemini")).strip().lower())
PY
)"
CLASSES_CSV="$(
  REPO_ROOT="$REPO_ROOT" python3 - <<'PY'
...
if [ "$LABEL_MODE" = "google" ]; then
  LABEL_MODE="gemini"
fi

if [ "$LABEL_MODE" != "gpt" ] && [ "$LABEL_MODE" != "gemini" ] && [ "$LABEL_MODE" != "codex" ]; then
  echo "Error: dispatch.sh supports label_mode=gpt, gemini, or codex (got: ${LABEL_MODE})"
  echo "For cua+sam use the dedicated label script."
  exit 1
fi
...
for i in $(seq 1 $NUM_AGENTS); do
  ...
  elif [ "$LABEL_MODE" = "gemini" ] && { [ -n "${GEMINI_API_KEY:-}" ] || [ -n "${GOOGLE_API_KEY:-}" ]; }; then
    codex exec --skip-git-repo-check --full-auto -C "$DIR" \
      "You are a labeling subagent. Label all frames in ${OUTPUT_DIR}/frames/ using the label skill.
       Run: uv run .agents/skills/label/scripts/run_batch.py
       Do not modify any other files. Only create .txt label files next to each .jpg." \
...
for i in $(seq 1 $NUM_AGENTS); do
  ...
  git branch -D "phase1_2/labeler-${i}" 2>/dev/null || true
done
