...
If the user wants to train a model, use the **phase1_2 skill** (`.agents/skills/phase1_2/SKILL.md`).
Ask for: YouTube URL (or local video path), target classes, **labeling mode** (cua+sam / gemini / gpt / codex), optional accuracy target.
Default to `label_mode=gemini` if the user does not specify.
...
If config.json is already populated, just run: `bash phase1_2.sh`
...
- Label modes: `cua+sam` (CUA clicks + SAM segmentation), `gemini` (default, native bbox), `gpt` (OpenAI fallback), `codex` (subagent image-view mode, no API keys)
...
3. **Frames but no labels** (no .txt files in frames/):
   ...
   - `cua+sam`: `uv run .agents/skills/label/scripts/label_cua_sam.py`
     - `gemini`: `uv run .agents/skills/label/scripts/run.py`
