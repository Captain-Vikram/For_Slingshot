...
open codex in the phase1_2 directory. it reads AGENTS.md automatically.
...
2. default to `label_mode=gemini` unless you choose another mode
3. write config.json
4. run the full pipeline: collect -> label -> augment -> train -> eval
5. loop if accuracy < target
...
### option 4: api mode (backend-ready)
...
- `POST /api/projects` to create/update config
- `POST /api/projects/{project}/run` to run `full` or one phase
- `GET /api/projects/{project}/status` for progress
- `GET /api/projects/{project}/events` for live logs (SSE)
- `GET /api/projects/{project}/metrics` and `/artifacts` for results
...
```json
{
  ...
  "label_mode": "gemini", // gemini (default) | gpt | codex | cua+sam
  "model": "gemini-2.5-flash", // active model for current label_mode
  "openai_model": "gpt-5-nano", // optional OpenAI fallback model
  "gemini_model": "gemini-2.5-flash", // optional explicit Gemini model
...
## provider selection (easy mode)

switch by changing only `label_mode` and (optionally) `model`:

```json
{ "label_mode": "gemini", "model": "gemini-2.5-flash" }
{ "label_mode": "gpt", "model": "gpt-5-nano" }
{ "label_mode": "codex" }
{ "label_mode": "cua+sam" }
```
...
2. create N git worktrees at `/tmp/phase1_2-workers/agent-{1..N}`
...
- **default path**: use `label_mode=gemini` with `gemini-2.5-flash`
- **openai fallback**: set `label_mode=gpt` and pick `gpt-5-nano` or `gpt-4.1-mini`
